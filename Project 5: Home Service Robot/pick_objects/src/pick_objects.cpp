...
int main(int argc, char** argv){
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
    ...
    ROS_INFO("The robot has failed to reach the pick up zone");
  ...
  goal_2.target_pose.pose.position.y = -1.0;
  ...
  // Check if the robot reached its goal
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
    ROS_INFO("The robot has reached the drop off point");
    nh.setParam("robot_at_dropoff_point", true);
  }
  else
    ROS_INFO("The robot has failed to reach the drop off zone");
